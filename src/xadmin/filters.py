...
@manager.register
class IsFreeTrialListFilter(BaseFilter):
    title = "is_trial"
    lookup_formats = {'exact': '%s__exact'}
    prefix_key = lookup_formats['exact'] % '_cus_istrial_'
    template = 'xadmin/filters/list.html'

    def __init__(self, request, params, model, admin_view, prefix_key=prefix_key):
        self.prefix_key = prefix_key
        super(IsFreeTrialListFilter, self).__init__(request, params, model, admin_view)
        if not bool(self.params):
            val = self.request.GET.get(self.prefix_key)
            if val:
                self.params = {self.prefix_key: val}
                self.used_params = {self.prefix_key: val}

    def has_output(self):
        return True

    def do_filte(self, queryset):
        val = self.used_params.get(self.prefix_key)
        if val == '1':
            return queryset.filter(type__type='purchase').prefetch_related('billings').filter(
                billings__transaction_id=F('billing_id'), billings__type=1, billings__status=2)
        elif val == '0':
            return queryset.filter(type__type='purchase').prefetch_related('billings').filter(
                billings__transaction_id=F('billing_id'), billings__type__in=(0, 1, 3), billings__status=1)
        return queryset
    ...
    def get_context(self):
        context = super(IsFreeTrialListFilter, self).get_context()
        context['choices'] = []
        for lookup, title in (
                ('', _('All')),
                ('1', _('Yes')),
            ...
            context['choices'].append(
                {
                    'selected': self.request.GET.get(self.prefix_key) == lookup,
                    'query_string': self.query_string({self.prefix_key: lookup}, [self.prefix_key, ]),
                    'display': title
                }
            )
...
@manager.register
class PkgListFilter(BaseFilter):
    title = "包"
    lookup_formats = {'exact': '%s__exact'}
    prefix_key = lookup_formats['exact'] % '_cus_pkg_'
    template = 'xadmin/filters/list.html'

    def __init__(self, request, params, model, admin_view, prefix_key=prefix_key):
        self.prefix_key = prefix_key
        super(PkgListFilter, self).__init__(request, params, model, admin_view)
        if not bool(self.params):
            val = self.request.GET.get(self.prefix_key)
            if val:
                self.params = {self.prefix_key: val}
    ...
    def do_filte(self, queryset):
        ...
        #     return queryset.filter(type__type='purchase').prefetch_related('billings').filter(
        #         billings__transaction_id=F('billing_id'), billings__type__in=(0, 1, 3), billings__status=1)
        return queryset.filter(pkg=val)
    ...
    def get_context(self):
        from apps.server.models import PkgInfo
        context = super(PkgListFilter, self).get_context()
        all_pkg = PkgInfo.objects.values_list('pkg', 'name')
        context['choices'] = [{
                    'selected': self.request.GET.get(self.prefix_key) == 'all',
                    'query_string': self.query_string({self.prefix_key: 'all'}, [self.prefix_key, ]),
                    'display': "ALL"
                },]
        for lookup, title in all_pkg:
            context['choices'].append(
                {
                    'selected': self.request.GET.get(self.prefix_key) == lookup,
                    'query_string': self.query_string({self.prefix_key: lookup}, [self.prefix_key, ]),
...
class TextFieldListFilter(FieldFilter):
    ...
    def test(cls, field, request, params, model, admin_view, field_path):
        return (
                ...
                or isinstance(field, models.TextField) or field.name=="user_country"
        )
