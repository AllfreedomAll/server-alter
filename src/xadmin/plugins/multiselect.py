...
class SelectMultipleTransfer(forms.SelectMultiple):
    ...
    def render(self, name, value, attrs=None, choices=(), renderer=None):
        ...
        if final_attrs.get('required', None):
...
class ListSelectPlugin(BaseAdminPlugin):

    def init_request(self, *args, **kwargs):
        return hasattr(self.admin_view, 'style_fields') and \
               (
                       'list_transfer' in self.admin_view.style_fields.values() or
                       'm2m_dropdown' in self.admin_view.style_fields.values()
               )

    def get_field_style(self, attrs, db_field, style, **kwargs):
        if style == 'list_transfer':
            return {'widget': ListSelectMultipleTransfer(db_field.verbose_name, False, choices=db_field.cus_choices),
                    'help_text': ''}
        return attrs


class ListSelectMultipleTransfer(forms.SelectMultiple):
    ...
    def __init__(self, verbose_name, is_stacked, attrs=None, choices=()):
        self.verbose_name = verbose_name
        self.is_stacked = is_stacked
        super(ListSelectMultipleTransfer, self).__init__(attrs, choices)
...
xadmin.site.register_plugin(ListSelectPlugin, ModelFormAdminView)
