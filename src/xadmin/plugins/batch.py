...
class BatchChangeAction(BaseActionView):
    ...
    def change_models(self, queryset, cleaned_data):
        ...
        if n:
            ...
            self.log('change',
                     _('Batch Changed %(count)d %(items)s.') % {"count": n, "items": model_ngettext(self.opts, n)})
            # log(self.request,'batch change','')
        ...
        if change_fields and self.request.POST.get('post'):
            ...
            if self.form_obj.is_valid():
                ...
                for i in change_fields:
                    if i not in self.request.POST:
                        self.form_obj.cleaned_data[i]=False
                if self.form_obj.cleaned_data:
                    self.change_models(queryset, self.form_obj.cleaned_data)
                    return None
...
class BatchSyncAction(BaseActionView):
    action_name = "sync_selected"
    description = ugettext_lazy(
        u'Batch Sync selected %(verbose_name_plural)s')

    batch_sync_form_template = None

    batch_fields = []

    # def sync_models(self, queryset, cleaned_data):
    def do_action(self, queryset):
        if not self.has_change_permission():
            raise PermissionDenied
        count = len(queryset)
        ...
        return TemplateResponse(self.request, self.batch_sync_form_template or
                                self.get_template_list('views/batch_sync_form.html'), context)

    @filter_hook
    def get_media(self):
        media = super(BatchSyncAction, self).get_media()
        media = media + self.vendor(
            'xadmin.page.form.js', 'xadmin.form.css')
        return media


class NewBatchSyncAction(BaseActionView):
    action_name = "sync_selected"
    description = ugettext_lazy(
        u'Batch Sync selected %(verbose_name_plural)s')
    ...
    def do_action(self, queryset):
        ...
        return TemplateResponse(self.request, self.batch_sync_form_template or
                                self.get_template_list('views/new_batch_sync_form.html'), context)

    @filter_hook
    def get_media(self):
        media = super(NewBatchSyncAction, self).get_media()
        media = media + self.vendor(
            'xadmin.page.form.js', 'xadmin.form.css')
        return media


class BatchChangeM2M(BaseActionView):
    action_name = "change_m2m"
    description = ugettext_lazy(
        u'Batch change m2m %(verbose_name_plural)s')
    ...
    def do_action(self, queryset):
        ...
        context = self.get_context()
        m2m = []
        for i in queryset.model._meta.many_to_many:
            m2m.append({"db_name": i.opts.db_table + "_" + i.name, 'qs': i.related_model.objects.all(),
                        'verbose_name': i.verbose_name, 'f_name': i.target_field.model._meta.model_name})
        context.update({
            "title": _("Batch %s") % objects_name,
            'queryset': queryset,
            'count': count,
            'm2m': m2m,
            'or_label': self.opts.model_name,

        })
        return TemplateResponse(self.request, self.batch_sync_form_template or
                                self.get_template_list('views/m2m_change.html'), context)

    @filter_hook
    def get_media(self):
        media = super(BatchChangeM2M, self).get_media()
        media = media + self.vendor(
            'xadmin.page.form.js',
            'xadmin.form.css',
...
class BatchTranslationAction(BaseActionView):
    action_name = "translation_selected"
    description = ugettext_lazy(
        u'Batch Translation selected %(verbose_name_plural)s')

    batch_translation_form_template = None
    ...
    def do_action(self, queryset):
        ...
        context.update({
            "title": _("Batch translation %s") % objects_name,
            'queryset': queryset,
            'count': count,

        })
        return TemplateResponse(self.request, self.batch_translation_form_template or
                                self.get_template_list('views/batch_tranlation_form.html'), context)

    @filter_hook
    def get_media(self):
        media = super(BatchTranslationAction, self).get_media()
        media = media + self.vendor(
            'xadmin.page.form.js', 'xadmin.form.css')
        return media
