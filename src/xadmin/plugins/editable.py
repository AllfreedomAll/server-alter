...
from xadmin.plugins.utils import log
from xadmin.plugins.images import AdminImageWidget
...
class EditPatchView(ModelFormAdminView, ListAdminView):
    ...
    def get(self, request, object_id):
        ...
        form.helper = helper
        if fields and isinstance(form.fields[fields[0]].widget, AdminImageWidget):
            s = '{% load i18n crispy_forms_tags %}<form method="post" action="{{action_url}}">' + \
                '<input type="checkbox" name="del_action" value="%s" />Delete' % fields[0] + \
    ...
    def post(self, request, object_id):
        ...
        if not fields:
            for key in request.POST.keys():
                field_name = key.rstrip("_1234567890")
        ...
        result = {}
        del_action = request.POST.get('del_action')
        if del_action:
            setattr(form.instance, del_action, None)
            form.save(commit=True)
            fields.append(del_action)
            result['result'] = 'success'
            result['new_data'] = None
            result['new_html'] = dict(
                [(f, self.get_new_field_html(f)) for f in fields])
            log(request, 'change', 'Changed %s.' % result['new_html'], self.org_obj)
