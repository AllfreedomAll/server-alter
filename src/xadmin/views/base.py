...
class CommAdminView(BaseAdminView):
    ...
    site_footer = getattr(settings, "XADMIN_FOOTER_TITLE", _(u"Kittenchat-company.inc"))
    ...
    def get_nav_menu(self):
        ...
        cus_order = {'apps:auth': 99, 'apps:xadmin': 100, 'apps:conf': 3, 'apps:device': 5, 'apps:account': 4,
                     'apps:server': 2, 'apps:payment': 6}
        ...
        site_menu.sort(key=lambda x: x['o'])
    ...
    def get_context(self):
        ...
        def check_selected(menu, path):
            ...
            if 'url' in menu:
                ...
                if chop_index == -1:
                    ...
                    if path.startswith('/space-x3/ads/'):

                        path_params = parse.parse_qs(parse.urlparse(path).query).get('_p_pkg__exact')
                        url_params = parse.parse_qs(parse.urlparse(menu['url']).query).get('_p_pkg__exact')
                        if path_params and path_params == url_params and path[0:15] == menu['url'][0:15]:
                            selected = True
        ...
        for menu in nav_menu:
            check_selected(menu, self.request.get_full_path())
