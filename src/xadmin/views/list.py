...
class ListAdminView(ModelAdminView):
    ...
    def get_context(self):
        ...
        new_context = {
            ...
            'add_url': self.model_admin_url('add')+"?"+self.request.META['QUERY_STRING'],
            'result_headers': self.result_headers(),
            'results': self.results()
        }
        if "/ads/clientads" in self.request.path:
            new_context['add_url'] = self.model_admin_url('add')+"ads-add/?"+self.request.META['QUERY_STRING']
        if "/ads/abtestads" in self.request.path:
            new_context['add_url'] = self.model_admin_url('add') + "abads-add/?" + self.request.META['QUERY_STRING']
    ...
    def result_item(self, obj, field_name, row):
        ...
        if (item.row['is_display_first'] and not self.list_display_links) \
            ...
            if self.list_display_links_details:
                ...
                url = self.url_for_result(obj) + "?" + self.request.META['QUERY_STRING'].replace("%", "%%")
