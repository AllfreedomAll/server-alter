...
class CreateAdminView(ModelFormAdminView):
    ...
    def post_response(self):
        ...
        if "_continue" in request.POST:
            ...
            return self.model_admin_url('change', self.new_obj._get_pk_val()) + "?" + request.META['QUERY_STRING']
        ...
        if "_addanother" in request.POST:
            ...
            if "_redirect" in request.POST:
                return request.POST["_redirect"] + "?" + request.META['QUERY_STRING']
            elif self.has_view_permission():
                return self.model_admin_url('changelist') + "?" + request.META['QUERY_STRING']
            else:
                return self.get_admin_url('index')


class DuplicateCreateCustomView(ModelFormAdminView):
    ...
    def get_context(self):
        ...
        context = super(DuplicateCreateCustomView, self).get_context()
        context.update(new_context)
        return context
...
class UpdateAdminView(ModelFormAdminView):
    ...
    def post_response(self):
        ...
        if "_continue" in request.POST:
            self.message_user(
                msg + ' ' + _("You may edit it again below."), 'success')
            return request.path + "?" + self.request.META['QUERY_STRING']
        elif "_addanother" in request.POST:
            self.message_user(msg + ' ' + (_("You may add another %s below.")
                                           % force_text(verbose_name)), 'success')
            return self.model_admin_url('add') + "?" + self.request.META['QUERY_STRING']
        else:
            self.message_user(msg, 'success')
            # Figure out where to redirect. If the user has change permission,
            # redirect to the change-list page for this object. Otherwise,
            # redirect to the admin index.
            if "_redirect" in request.POST:
                return request.POST["_redirect"] + "?" + self.request.META['QUERY_STRING']
            elif self.has_view_permission():
                ...
                return change_list_url + "?" + self.request.META['QUERY_STRING']
            else:
                return self.get_admin_url('index')
...
class AdsCusEditView(ModelFormAdminView):
    ...
    def get_context(self):
        ...
        context = super(AdsCusEditView, self).get_context()
        context.update(new_context)
        return context
    ...
    def get_breadcrumb(self):
        bcs = super(ModelFormAdminView, self).get_breadcrumb()
        bcs[1]['url'] = bcs[1]['url'] + "?" + self.request.get_full_path().split("?")[-1]
        edit_org_obj_info = {"包": self.org_obj.pkg, '国家': self.org_obj.countries}
        item = {'title': _('Edit: %s %s') % (force_text(self.opts.verbose_name), edit_org_obj_info)}
        if self.has_add_permission():
            item['url'] = self.model_admin_url('add')
        bcs.append(item)
        return bcs
    ...
    def get_response(self):
        ...
        from apps.ads.models import AdsSite,PKG_INFO
        context = self.get_context()
        context.update(self.kwargs or {})
        context['ads_content'] = context['form'].instance.content
        context['edit_id'] = self.org_obj.id
        pkg = self.org_obj.pkg
        os = None
        for i in PKG_INFO:
            if pkg == i[0]:
                os = i[2]
                break
        context['ads_site'] = list(AdsSite.objects.filter(os=os).values_list("name", flat=True))
        return render(self.request, './xadmin/views/ads_list.html', context)
        # return TemplateResponse(
        #     self.request, self.add_form_template or self.get_template_list(
        #         'views/model_form.html'),
        #     context)
...
class AdsABEditView(AdsCusEditView):
    @filter_hook
    def get_response(self):
        from django.shortcuts import render
        from apps.ads.models import AdsSite
        context = self.get_context()
        context.update(self.kwargs or {})
        context['ads_content'] = context['form'].instance.content
        context['ctr'] = [(i.id, i) for i in context['form'].fields['countries'].queryset]
        context['edit_id'] = self.org_obj.id
        pkg_os = self.org_obj.pkg.os
        context['ads_site'] = list(AdsSite.objects.filter(os=pkg_os).values_list("name", flat=True))
        return render(self.request, './xadmin/views/abads_list.html', context)


class AdsCreateAdminView(ModelFormAdminView):
    ...
    def get_context(self):
        ...
        context = super(AdsCreateAdminView, self).get_context()
        context.update(new_context)
        return context
    ...
    def get_response(self):
        from django.shortcuts import render
        from apps.ads.models import AdsSite,PKG_INFO
        # from apps.conf.models import PKG
        context = self.get_context()
        context.update(self.kwargs or {})
        context['ctr'] = [(i.id, i) for i in context['form'].fields['countries'].queryset]
        pkg_name = self.request.GET.get("_p_pkg__exact")
        os = None
        for i in PKG_INFO:
            if pkg_name == i[0]:
                os = i[2]
        ...
        # pkg_os = PkgInfo.objects.get(pk=self.request.GET.get("_p_pkg__id__exact")).os
        context['ads_site'] = list(AdsSite.objects.filter(os=os).values_list("name", flat=True))
        return render(self.request, './xadmin/views/ads_add.html', context)
...
class AdsABCreateAdminView(AdsCreateAdminView):
    @filter_hook
    def get_response(self):
        from django.shortcuts import render
        from apps.ads.models import AdsSite
        from apps.conf.models import PKG
        # from apps.other_conf.models import PkgInfo
        context = self.get_context()
        context.update(self.kwargs or {})
        context['ctr'] = [(i.id, i) for i in context['form'].fields['countries'].queryset]
        pkg_name = self.request.GET.get("_p_pkg__exact")
        # pkg_os = PkgInfo.objects.get(pk=self.request.GET.get("_p_pkg__exact")).os
        context['ads_site'] = list(AdsSite.objects.filter(os=pkg_name).values_list("name", flat=True))

        return render(self.request, './xadmin/views/abads_add.html', context)
